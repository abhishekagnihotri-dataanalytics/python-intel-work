...
# __schedule__ = "Weekly Sunday 4.30PM PST"
...
from Helper_Functions import uploadDFtoSQL, executeStoredProcedure
...
connXeus_F32 = PyUber.connect(datasource="F32_PROD_XEUS")

####import availability data from XEUS and land it in staging table - start####
data_availability= connXeus_F32.execute("""SELECT
a.WW
,a.AREA
,a.CEID
,b.FACILITY
...
rows = data_availability.fetchall()
names = [x[0] for x in data_availability.description]
df_availability = pd.DataFrame(rows, columns=names)

####import availability data from XEUS and land it in staging table - start####
data_availability_process = connXeus_F32.execute("""
Select 
a.WW
,a.ENTITY
...
data_ToolProcess = connXeus_F32.execute("""SELECT 
distinct
b.FACILITY
,a.ENTITY
...
data_GSC_Config = connXeus_F32.execute("""SELECT
a.ENTITY, 
a.FACILITY,
a.PSPEC_TARGET,
...
data_Availability_NFA_stg = connXeus_F32.execute("""
SELECT
eah.WW
,eah.AREA
,eah.CEID
...
data_CEID_Decoder = connXeus_F32.execute("""SELECT a.owned_by, a.entity, a.CEID, a.REV_CEID, a.rollup_end_time
...
data_CEID_Decoder_sq = connXeus_F32.execute("""select a.entity, a.ceid, a.owned_by, MAX(a.rollup_end_time) Max_Rollup_End_Time, e.facility, current_date as RunDate
 FROM LIMA_F_ENTITY_AVAILABILITY_HR a
...
###Move data from Stage to Base####
sp_name = '[ToolAvailability].[Load_XEUS_Availability]'
sp_succeeded, error_msg = executeStoredProcedure(sp_name, server='sql2377-fm1-in.amr.corp.intel.com,3180', database='SCDA')
log(sp_succeeded, project_name=project_name, data_area=data_area, row_count=0, error_msg=error_msg)


log('I', project_name=project_name, data_area='XEUS F32 Data', row_count=0)
