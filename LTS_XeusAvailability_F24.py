...
# __schedule__ = "Weekly Sunday 2.30PM PST"
...
connXeus_F24 = PyUber.connect(datasource="F24_PROD_XEUS")


####import availability data from XEUS and land it in staging table - start####
data_availability = connXeus_F24.execute("""SELECT
a.WW
,a.AREA
,a.CEID
,b.FACILITY
...
####import availability data from XEUS and land it in staging table - start####
data_availability_process = connXeus_F24.execute("""
Select 
a.WW
,a.ENTITY
...
data_ToolProcess = connXeus_F24.execute("""SELECT 
distinct
b.FACILITY
,a.ENTITY
...
data_GSC_Config = connXeus_F24.execute("""SELECT
a.ENTITY, 
a.FACILITY,
a.PSPEC_TARGET,
...
data_Availability_NFA_stg = connXeus_F24.execute("""
SELECT
eah.WW
,eah.AREA
,eah.CEID
...
data_CEID_Decoder = connXeus_F24.execute("""SELECT a.owned_by, a.entity, a.CEID, a.REV_CEID, a.rollup_end_time
...
data_CEID_Decoder_sq = connXeus_F24.execute("""select a.entity, a.ceid, a.owned_by, MAX(a.rollup_end_time) Max_Rollup_End_Time, e.facility, current_date as RunDate
 FROM LIMA_F_ENTITY_AVAILABILITY_HR a
...
log('I', project_name=project_name, data_area='XEUS F24 Data', row_count=0)
