        ...
        for subject in ariba_files:
            ...
            while retries < 3:
                try:
                    ...
                    print('Checkign if the file {} already exists'.format(str(change_file_name)+".x"))
                    if os.path.isfile(os.path.join(shared_drive_folder_path, str(change_file_name))+".xlsx"):
                        print('moving file {} to archive'.format(str(change_file_name) + ".xlsx"))
                        try:
                            shutil.move(os.path.join(shared_drive_folder_path, str(change_file_name))+".xlsx", os.path.join(shared_drive_folder_path,'Archive',change_file_name)+'_'+str(datetime.now().strftime('%Y_%m_%d_%H_%M_%S'))+'.xlsx')
